...
{
    void *new_data_address = calloc(vec->esize, vec->count + VECTOR_ELEMENT_INCREMENT);
    memcpy(new_data_address, vec->data, vector_total_size(vec));
    vector *new_vec = calloc(sizeof(vec), 1);
    memcpy(new_vec, vec, sizeof(vec));
...
void vector_save(vector *vec)
{
    ...
    tmp_vec.saves = NULL;
    vector_push(vec->saves, &tmp_vec);
}

void vector_restore(vector *vec)
{
    vector save_vec = *((vector *)(vector_back(vec->saves)));
    save_vec.saves = vec->saves;
    *vec = save_vec;
    vector_pop(vec->saves);
}
